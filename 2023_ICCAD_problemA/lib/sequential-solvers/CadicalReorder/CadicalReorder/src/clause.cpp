...
void Internal::reorder_initial_branches(){

  vector<int> tmp_cluster;

  for (const auto c : clauses){
    for (const_literal_iterator lit = c->begin (); lit != c->end (); lit++){
      Flags & f = flags (*lit);
      if (!f.seen) {
        tmp_cluster.push_back(abs(*lit));
        f.seen = true;
      }
    }
  }

  for(std::vector<int>::iterator it = tmp_cluster.end() - 1; it >= tmp_cluster.begin(); it--) bump_queue (*it); 
  
  for(const auto lit : tmp_cluster){
    Flags & f = flags (lit);
    f.seen = false;
  }
