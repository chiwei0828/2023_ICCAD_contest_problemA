...
static void
init_CCAnr_adapter (kissat * solver)
{
  solver->lssolver = kissat_malloc(solver, sizeof(CCAnr));
  solver->lssolver->ccanr_has_constructed = false;
  solver->lssolver->ls_best_unsat_num = INT_MAX;
  solver->lssolver->restarts_gap = 300;
  solver->lssolver->restarts_basic = 300;
  solver->lssolver->freeze_ls_restart_num = 0;
  solver->lssolver->max_trail = 0;
  solver->lssolver->solved_by_ls = false;
  solver->lssolver->cls_ct = 0;
  solver->lssolver->reward = GET_OPTION(walkccnrreward);
}
...
rephase_walking (kissat * solver)
{
  ...
  STOP(rephase);
  if (GET_OPTION( walkccnr )) {
    if (solver->lssolver == NULL) {
      init_CCAnr_adapter(solver);
    }
    call_ls(solver, solver->lssolver);
  } else
    kissat_walk(solver);
  START(rephase);
