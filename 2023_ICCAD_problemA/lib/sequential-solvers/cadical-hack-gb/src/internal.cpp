...
void Internal::track_glue_var(vector<int>& c){
  num_glue++;
  for(int i=0;i<c.size();i++)
  {
    ...
    glue_var[v] =true;
    glue_centrality[v]++;
    total_glue_var_count++;
  }
}

void Internal::bump_glue_var(const int v){
  glue_var[v]=false;
  double old = score (v);
  score(v) = old + old *((double) glue_centrality[v]/ (double) total_glue_var_count);
  if (scores.contains (v)) scores.update (v);
}
