...
int Internal::walk_round (int64_t limit, bool prev) {
  ...
  if (doslsfirst) {failed = false; res = 0;}
  //int64_t broken;
  if (!failed) {
    //int64_t broken = walker.broken.size ();
    int64_t broken;
    ...
    if (doslsfirst) copy_phases (phases.saved); 
    else walk_save_minimum (walker);

    int64_t flips = 0, minimum = broken;

    while (doslsfirst ? 1 : (!terminated_asynchronously () &&
           !walker.broken.empty () &&
	  ...
	  else copy_phases (phases.prev);

      walk_save_minimum (walker);
      if(doslsfirst) { if (flips > limit || broken == 0) {break;} }
    }
  ...
  if (!doslsfirst) copy_phases (phases.prev);
