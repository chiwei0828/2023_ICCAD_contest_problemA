...
void Internal::analyze () {
  ...
  for (const auto & lit : analyzed) {
	  const Var & v = var (lit);
      assert (val (lit));  
      Clause * mid_reason = v.reason;
	  if(mid_reason){
		if(mid_reason->redundant) 
			num_re++;
		else
			num_ir++;
	  }
  ...
  Clause * end_reason = conflict;
  int ori_clearnt_type = 0;
  if(end_reason->redundant) {
	  num_re++;
	  ori_clearnt_type = 0;
  } else {
	  num_ir++;
  ...
  ir_rate = (1.0*num_ir)/(1.0*(num_re + num_ir)); 
  
  typeEnd  = ori_clearnt_type;
  ori_rate = ir_rate;
