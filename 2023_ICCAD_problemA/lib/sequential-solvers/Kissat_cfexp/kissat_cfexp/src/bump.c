...
//static void
//rescale_scores_exp (kissat * solver, heap * scores)
//{
//    INC (rescaled);
//    const double max_score = kissat_max_score_on_heap (scores);
//    kissat_phase (solver, "rescale exp", GET (rescaled),
//                  "maximum score %g increment %g", max_score, solver->scinc);
//    const double rescale = MAX (max_score, solver->scinc1);
//    assert (rescale > 0);
//    const double factor = 1.0 / rescale;
//    kissat_rescale_heap (solver, scores, factor);
//    solver->scinc1 *= factor;
//    kissat_phase (solver, "rescale",
//                  GET (rescaled), "rescaled by factor %g", factor);
//}
...
unsigned *sort_base;
int ils_cmpfunc (const void * a, const void * b)
{
   return ( sort_base[*(unsigned*)a] - sort_base[*(unsigned*)b] );
}


void infols_bump_update(struct kissat* solver){
    infols *ils = &solver->ils;
    if(solver->vars != ils->var_nums || !solver->stable) return;
    heap *scores = &solver->scores;
    flags *flags = solver->flags;
    for(unsigned idx=0;idx<ils->conflicted_sz;++idx){
        unsigned var = ils->conflicted[idx];
        if(flags[var].active){
            const double old_score = kissat_get_heap_score (scores, var);
            const double new_score = old_score + solver->scinc*ils->conflicts_num[var];
            kissat_update_heap(solver,scores,var,new_score);
            if (new_score > MAX_SCORE) rescale_scores (solver, scores);
        }
...
void updateHeapWithExpScore(struct kissat * solver){
    expscore *exps = &solver->exps;
    heap *scores = &solver->scores;
    flags *flags = solver->flags;
    for(unsigned idx=0;idx<exps->var_num;++idx){
        unsigned var = exps->varInd[idx];
        if(flags[var].active){
            const double old_score = kissat_get_heap_score (scores, var);
            const double new_score = old_score + solver->scinc*solver->para_expScore*exps->expScore[var];
            //if(exps->expScore[var]!=0 || solver->scinc!=0) printf("bump.c 184:  old_score: %f,   solver->scinc: %f,    expScore: %f. \n", old_score, solver->scinc, 50*exps->expScore[var]);
            kissat_update_heap(solver,scores,var,new_score);
            if (new_score > MAX_SCORE) rescale_scores (solver, scores);
        }
