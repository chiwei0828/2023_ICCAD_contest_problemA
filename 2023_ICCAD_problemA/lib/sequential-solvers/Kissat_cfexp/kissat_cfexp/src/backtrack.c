...
kissat_backtrack (kissat * solver, unsigned new_level)
{
  ...
  //frame *new_frame_ = &FRAME (new_level);printf("backtrack.c 96: decision: %u, new_size_: %u \n", new_frame_->decision, new_frame_->trail);
  unsigned j = new_size;//printf("backtrack.c 97: decision: %u, new_size: %u \n", new_frame->decision, new_size);
  if (solver->stable)
    {
      ...
      for (unsigned i = j; i != old_size; i++)
	{
      ...
      //printf("backtrack.c 109: value: %u, lit: %u, level: %u\n",(solver->values)[lit], lit, level);
	  if (level <= new_level)
	    {
	      trail[j++] = lit;
...
void
kissat_backtrack_exp (kissat * solver, unsigned new_level)
{
    assert (solver->level >= new_level);
    if (solver->level == new_level)
        return;
