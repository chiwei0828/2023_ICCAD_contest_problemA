...
//  expscore *exps = &solver->exps;
  if(triggerExploration(solver)){
      //int old_unassigned = solver->unassigned;
      explore(solver);
      computeExplorationScore(solver);
      updateHeapWithExpScore(solver);
      //printf("decide.c 196: unassigned: %d, old_unassigned: %d!!!\n", solver->unassigned, old_unassigned);
  }
  solver->level++;
