...
kissat_learn_clause (kissat * solver)
{
  ...
  //lbd
  solver->new_clause_lbd = glue;
  ++solver->clause_num;
  solver->avg_lbd = (solver->avg_lbd * (solver->clause_num-1) + (double) glue) / (double) solver->clause_num;
...
//
//static void
//learn_binary_exp (kissat * solver)
//{
//    const unsigned *lits = BEGIN_STACK (solver->clause.lits);
//    const unsigned not_uip = lits[0];
//    const unsigned other = lits[1];
...
//
//static void
//learn_reference_exp (kissat * solver)
//{
//    assert (solver->level > 1);
//    unsigned *lits = BEGIN_STACK (solver->clause.lits);
//    const unsigned not_uip = lits[0];
...
void
kissat_learn_clause_exp (kissat * solver)
{
    if (!solver->probing)
        INC (learned);
    if (solver->stable)
...
//    assert (size > 0);
//    if (size == 1)
//        learn_unit_exp (solver);
//    else if (size == 2)
//        learn_binary_exp (solver);
//    else
//        learn_reference_exp (solver);
}