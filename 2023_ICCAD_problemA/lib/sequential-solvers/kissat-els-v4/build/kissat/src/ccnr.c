...
void alloc_memory(CCAnr *lssolver){
	...
	// adapter
	lssolver->tmp_up_build_soln = (char*)malloc(sizeof(char)*var_mem);
	lssolver->ls_mediation_soln = (char*)malloc(sizeof(char)*var_mem);
	lssolver->top_trail_soln = (char*)malloc(sizeof(char)*var_mem);
	lssolver->ls_best_soln = (char*)malloc(sizeof(char)*var_mem);
...
{
	int i;
	for (i = 0; i < lssolver->old_num_clauses; i++) 
	{
		free(lssolver->clause_lit[i]);
	}
	
	for(i=1; i<=lssolver->old_num_vars; ++i)
	{
	...
	// adapter
	free(lssolver->tmp_up_build_soln);
	free(lssolver->ls_mediation_soln);
	free(lssolver->top_trail_soln);
	free(lssolver->ls_best_soln);
}
...
int pick_var(CCAnr *lssolver)
{
	...
	if(lssolver->goodvar_stack_fill_pointer>0)
	{
		...
		lssolver->limit_step+=lssolver->goodvar_stack_fill_pointer;
	...
	lssolver->limit_step+=lssolver->unsatvar_stack_fill_pointer;
	...
	lssolver->limit_step+=lssolver->clause_lit_count[c];
...
bool local_search(CCAnr *lssolver)
{
	...
	for (lssolver->step = 0; lssolver->step<lssolver->max_flips; lssolver->step++)
	{
		...
		if(lssolver->limit <= lssolver->limit_step) return false;
...
static inline void unsat(CCAnr *lssolver, int clause)
{
...
static inline void sat(CCAnr *lssolver, int clause)
{
...
void init(CCAnr *lssolver,char *soln)
{
	...
	lssolver->init_unsat_cnt = lssolver->unsat_stack_fill_pointer;
}
...
void update_after_build(CCAnr *lssolver){
	int i,c,v;
	assert(lssolver->num_clauses >= lssolver->cls_ct);
	if (lssolver->num_clauses > lssolver->cls_ct)
		lssolver->num_clauses = lssolver->cls_ct;
...
void smooth_clause_weights(CCAnr *lssolver)
{
	...
	lssolver->limit_step+=lssolver->num_clauses;
...
void update_clause_weights(CCAnr *lssolver)
{
	...
	lssolver->limit_step+=lssolver->unsat_stack_fill_pointer+lssolver->unsatvar_stack_fill_pointer;
...
void confl_trans(CCAnr* lssolver){
	...
	for(int i=1;i<=lssolver->num_vars;++i){
		lssolver->conflict_ct[i] = lssolver->conflict_ct[i]*lssolver->reward/lssolver->step;
