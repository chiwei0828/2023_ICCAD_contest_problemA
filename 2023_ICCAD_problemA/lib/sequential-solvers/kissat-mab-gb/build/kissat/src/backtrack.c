...
{
  solver->is_glue_var_vsids[v]=false;
  heap *scores = &solver->scores;
  solver->gbumped_vsids++;
  const double old_score = kissat_get_heap_score (scores, v);
  double bf = (double) solver->in_glue_vsids[v] / (double) solver->stable_g_vsids; /// (double) solver->stable_g_vsids; //(double) solver->total_in_glue_vsids; //(double) solver->stable_g;
  const double new_score = old_score + old_score*bf;
  //printf("VSIDS ====>%d %f %f %f\n",v, bf, old_score, new_score);
