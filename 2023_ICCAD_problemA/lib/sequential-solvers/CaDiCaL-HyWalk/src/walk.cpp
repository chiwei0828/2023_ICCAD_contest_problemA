...
/*------------------------------------------------------------------------*/
int SW(int P1, int P2, double P3, int P4, double P5, int P6){
    if(P4<=2.9){
        if(P1<=361632){
            if(P3<=27.956){
                if(P2<=1163963){
...
int Internal::walk_round (int64_t limit, bool prev) {
  ...
  if (stats.walk.count == 1){
    ...
    for (const auto c : clauses) {
      if (c->garbage || (c->redundant && (!opts.walkredundant || !likely_to_be_kept_clause (c)))) continue;
      C++;
      int T = c->size;
      if (T > P6) P6 = T;
      if (T < P4) P4 = T;
