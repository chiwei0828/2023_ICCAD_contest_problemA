...
kissat_increase_size (kissat * solver, unsigned new_size)
{
  ...
  if(solver->heuristic_focus == 1) {
    CREALLOC_VARIABLE_INDEXED (unsigned,conflicted_lrb);
    CREALLOC_VARIABLE_INDEXED (unsigned,picked);
    for (int i = 0; i < new_size; i++) solver->picked[i] = solver->conflicted_lrb[i] = 0;
  }
...
kissat_decrease_size (kissat * solver)
{
  ...
  if(solver->heuristic_focus==1) {
     NREALLOC_VARIABLE_INDEXED (unsigned, conflicted_lrb);
     NREALLOC_VARIABLE_INDEXED (unsigned, picked);
  }
  if(solver->heuristic==1 || solver->mab)
