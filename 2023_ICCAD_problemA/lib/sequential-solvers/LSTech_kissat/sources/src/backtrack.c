...
{
  assert(solver->stable || solver->heuristic_focus == 1);
  const unsigned idx = IDX(lit);
...
void kissat_backtrack(kissat *solver, unsigned new_level)
{
  ...
  else
  {
    ...
    else if (solver->heuristic_focus == 1){
      heap *scores = &solver->scores_lrb;
      for (unsigned i = j; i != old_size; i++)
      {
        const unsigned lit = trail[i];
        const unsigned idx = IDX(lit);
        ...
        else
        {
          unassign(solver, values, lit);
          add_unassigned_variable_back_to_heap(solver, scores, lit);
          unsigned age = solver->statistics.conflicts - solver->picked[idx];
          if (age > 0)
          {
            double reward_lrb = 1.0 * solver->conflicted_lrb[idx] / age;
            double old_score = kissat_get_heap_score(&solver->scores_lrb, idx);
            double new_score = solver->step_lrb * reward_lrb + (1 - solver->step_lrb) * old_score;
            //printf("c %.6f -> %.6f\n", old_score, new_score);
            LOG("new score[%u] = %g vs %g",
                v, new_score, old_score);
            kissat_update_heap(solver, &solver->scores_lrb, idx, new_score);
          }
          // solver->canceled[idx] = solver->statistics.conflicts;
          unassigned++;
        }
      }
    }
