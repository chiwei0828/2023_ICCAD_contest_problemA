...
sort_stable_probes(kissat *solver, unsigneds *probes)
{
  ...
  const heap *scores;
  if (solver->stable) 
    scores = solver->heuristic == 0 ? &solver->scores : &solver->scores_chb;
...
{
  if (solver->stable || solver->heuristic_focus == 1)
            ...
            unsigned *failed_ptr, uint64_t *resolved_ptr)
{
  ...
  while (!EMPTY_STACK(*probes))
  {
    ...
    else if (!solver->heuristic_focus)
      LOG("probing %s{%u}", LOGLIT(probe), LINK(IDX(probe)).stamp);
    else if (solver->heuristic_focus == 1)
      LOG("probing %s[%g]", LOGLIT(probe),
          kissat_get_heap_score (&solver->scores_lrb, IDX (probe)));
    probed++;
