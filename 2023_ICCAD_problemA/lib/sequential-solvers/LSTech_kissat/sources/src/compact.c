...
compact_literal(kissat *solver, unsigned dst_lit, unsigned src_lit)
{
  ...
  if (solver->heuristic_focus == 1) {
    solver->conflicted_lrb[dst_idx] = solver->conflicted_lrb[src_idx];
    solver->picked[dst_idx] = solver->picked[src_idx];
  }
...
void kissat_finalize_compacting(kissat *solver, unsigned vars, unsigned mfixed)
{
  ...
  if (solver->heuristic_focus == 1)
  {
    memset(solver->conflicted_lrb + vars, 0, reduced * sizeof(unsigned));
    memset(solver->picked + vars, 0, reduced * sizeof(unsigned));
  }
  if (solver->heuristic == 1 || solver->mab)
