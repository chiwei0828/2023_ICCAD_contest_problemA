...
kissat_init(void)
{
  ...
  solver->heuristic_focus = GET_OPTION(heuristic_focus);
  solver->partial_sat = 0;
  solver->initshuffle = -1;
  solver->bump_one = -1;
  solver->watching = true;
  solver->conflict.size = 2;
  solver->rephase_type = GET_OPTION(rephasetype);
...
{
  // printf("c shuffle:%d bump:%d target:%d\n", solver->initshuffle, solver->bump_one, GET_OPTION(target));
  // if (!solver->rephase_type) printf("c rephase use target-rephase\n");
  // if (solver->rephase_type == 1) printf("c rephase use rand-rephase\n");
  // if (solver->partial_sat == 1) printf("c Use Partial SAT\n");
  // if (!solver->heuristic_focus) printf("c Focus branch use VMTF\n");
  // if (solver->heuristic_focus == 1) printf("c Focus branch use LRB\n");
  kissat_require_initialized(solver);
