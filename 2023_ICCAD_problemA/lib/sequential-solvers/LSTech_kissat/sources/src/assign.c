              ...
              bool binary, bool redundant, unsigned level, unsigned reason)
{
  ...
  if (!solver->stable && (solver->heuristic_focus == 1)) {
    solver->picked[idx] = solver->statistics.conflicts;
    solver->conflicted_lrb[idx] = 0;
    // int age = solver->statistics.conflicts - solver->canceled[idx];
    // if (age > 0) {
    //   double decay = pow(0.95, age);
    //   double old_score = kissat_get_heap_score(&solver->scores_lrb, idx);
    //   double new_score = old_score * decay;
