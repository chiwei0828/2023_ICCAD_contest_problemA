...
parse_options(application *application, int argc, char **argv)
{
  ...
  for (int i = 1; i < argc; i++)
  {
    ...
    else if (!strcmp(arg, "--psat"))
      solver->partial_sat = 1;
    else if ((valstr = kissat_parse_option_name(arg, "branch_focus")))
    {
      if (!strcmp(valstr, "vmtf") || !strcmp(valstr, "VMTF") || !strcmp(valstr, "Vmtf"))
        application->solver->heuristic_focus = 0;
      else if (!strcmp(valstr, "lrb") || !strcmp(valstr, "LRB") || !strcmp(valstr, "Lrb"))
        application->solver->heuristic_focus = 1;
      // else if (!strcmp(valstr, "chb") || !strcmp(valstr, "CHB") || !strcmp(valstr, "Chb"))
      //   application->solver->heuristic = 2;
      else
        ERROR("invalid argument in '%s' (try '-h')", arg);
    }
    else if ((valstr = kissat_parse_option_name(arg, "rephase")))
    {
      if (!strcmp(valstr, "target-rephase") || !strcmp(valstr, "TARGET-REPHASE") || !strcmp(valstr, "Target-rephase"))
        application->solver->rephase_type = 0;
      else if (!strcmp(valstr, "rand-rephase") || !strcmp(valstr, "RAND-REPHASE") || !strcmp(valstr, "Rand-rephase"))
        application->solver->rephase_type = 1;
      else
        ERROR("invalid argument in '%s' (try '-h')", arg);
    }
...
void kissat_mab_parse(struct kissat *solver)
{
  ...
  solver->step_lrb = 0.1 * GET_OPTION(stepchb);
