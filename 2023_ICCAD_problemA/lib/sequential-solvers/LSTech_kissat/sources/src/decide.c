...
kissat_next_decision_variable(kissat *solver)
{
  ...
  else if (solver->heuristic_focus == 0)
    res = last_enqueued_unassigned_variable(solver);
  else if (solver->heuristic_focus == 1)
    res = largest_score_unassigned_variable(solver,&solver->scores_lrb);
...
decide_phase(kissat *solver, unsigned idx)
{
  ...
  if (force)
    ...
    if (solver->partial_sat == 1 && rand() % 3 < 2) target = 1;
  }
