			  ...
			  unsigned lit, const char *type)
{
  ...
  if(a->redundant){
	  solver->num_analyze_learntClause += 1;    
  }
  else {
	  solver->num_analyze_oriClause += 1;    
  }
...
reset_analyze (kissat * solver)
{
  ...
  //=============================================
  solver->num_analyze_oriClause    = 0;
  solver->num_analyze_learntClause = 0;	
  solver->typeStart = 0;      
  solver->typeEnd   = 0;
  solver->ori_rate  = 0.0;
  //==============================================
...
kissat_analyze (kissat * solver, clause * conflict)
{
  ...
  //solver->typeStart;
  assert(conflict);
  if(!(conflict->redundant)) solver->typeEnd = 1;
  //==============================
  int res;
  do
    {
      ...
      else
	{
	  ...
	  //==============================================      
	  int oriNum = solver->num_analyze_oriClause;
	  int learnNum = solver->num_analyze_learntClause;
	  if(oriNum + learnNum)
		{
		  float ori_rate = 1.0*(oriNum)/(1.0*(oriNum + learnNum));
		  solver->ori_rate = ori_rate;	  
		}
	  //==============================================
