...
{  
  ...
  //=================================
  /*
  res = local_search_round_first (solver);
  if(res == 10)
	  { 
	    printf("first sls do end 03-4 !\n");		
        return res;
	  }
  */
  //=================================
...
int
local_search_round_first (kissat * solver)
{
  INC (walks);
  litpairs irredundant;
  litwatches redundant;
  ...
  int flipFirstNum = 9000000;
  int res = 0;
  int current_first = 0;
  while (flipFirstNum)
    {      
      local_search_step (solver, &walker);
	  //核实UNSAT子句数是否为零。为零退出并返回10
	  current_first = currently_unsatified (&walker);
	  if(!current_first)
		  {
		     res = 10;
			 printf("res = 10 \n");
