...
void init_cdata (kissat * solver)
{
  ...
  solver->is_glue_var_chb = calloc(solver->vars,sizeof(int)); 
  solver->is_decision_chb = calloc(solver->vars,sizeof(int)); 
  solver->in_glue_chb =  calloc(solver->vars,sizeof(unsigned)); 
  solver->total_in_glue_chb = 0;
  solver->max_bf = -1;
  solver->gbumped_chb = 0;
  solver->stable_g_chb = 0;

  solver->is_glue_var_vsids = calloc(solver->vars,sizeof(int)); 
  solver->is_decision_vsids = calloc(solver->vars,sizeof(int)); 
  solver->in_glue_vsids =  calloc(solver->vars,sizeof(unsigned)); 
  solver->total_in_glue_vsids = 0;
  solver->gbumped_vsids = 0;
  solver->stable_g_vsids = 0;
...
kissat_print_statistics (kissat * solver)
{
  ...
  if(solver->mab) {
	...
	printf("\n");
  printf ("c stable CDP stats: %d %d %d %d %f %d\n", GET_OPTION (cdphaseaction), GET_OPTION (cdphaseactionh), GET_OPTION (substantialcdfactor) ,solver->cdata.num_cdp, (double) solver->cdata.total_cdp_len / (double) solver->cdata.num_cdp, solver->cdata.action_applied_amid_cd);
  printf ("c random selection stats: %d %d %f %f %f %f %f %f %f\n", 
            solver->rand_selection,
            solver->cdata.rand_dec_confs, 
            (double) solver->cdata.decide_rand_success / (double) solver->cdata.action_applied_amid_cd,
            (double) solver->cdata.decide_rand_success_tries / (double) solver->cdata.decide_rand_success,
            (double) solver->cdata.rand_dec_confs / (double) solver->rand_selection,
            (double) solver->cdata.rand_dec_learn_glue / (double) solver->cdata.rand_dec_confs,
            (double) solver->total_glue / (double) solver->total_learnt,
            (double) solver->cdata.rand_dec_learn_size / (double) solver->cdata.rand_dec_confs,
            (double) solver->total_size / (double) solver->total_learnt  );
  }
#ifndef NPROOFS
  if (solver->proof)
    {
