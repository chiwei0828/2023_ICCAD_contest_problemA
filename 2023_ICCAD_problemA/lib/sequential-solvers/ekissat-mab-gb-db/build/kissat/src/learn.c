...
void
track_glu_var_vsids (kissat *solver)
{
  for(unsigned i=0;i<SIZE_STACK(solver->clause.lits);i++)
  {
    unsigned var = IDX(PEEK_STACK (solver->clause.lits, i));
    solver->is_glue_var_vsids [var]= 1;
    solver->in_glue_vsids [var]++;
    solver->total_in_glue_vsids++;    
  }
}

void
track_glu_var_chb (kissat *solver)
{
  for(unsigned i=0;i<SIZE_STACK(solver->clause.lits);i++)
  {
    unsigned var = IDX(PEEK_STACK (solver->clause.lits, i));
    solver->is_glue_var_chb [var]=1;
    solver->in_glue_chb [var]++;
    solver->total_in_glue_chb++;    
  }
}
...
kissat_learn_clause (kissat * solver)
{
  ...
  {
    //ADD(num_glue,1);
    if(solver->stable) // only for VSIDS
    {
      if (solver->heuristic == 0)
      {
        track_glu_var_vsids (solver);
        solver->stable_g_vsids++;
      }
      else
      {
        track_glu_var_chb (solver);
        solver->stable_g_chb++;
      }
    }
  }
  ...
  if (solver->cdata.decide_rand)
  {
    ...
    solver->cdata.rand_dec_learn_glue += SIZE_STACK (solver->levels);
    solver->cdata.rand_dec_learn_size += SIZE_STACK (solver->clause.lits);
  }
  solver->total_glue += SIZE_STACK (solver->levels);
  solver->total_size += SIZE_STACK (solver->clause.lits);
  solver->total_learnt++;
}
