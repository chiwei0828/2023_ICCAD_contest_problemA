...
bool invalid_lit (kissat * solver, unsigned res)
{
  unsigned lit = LIT (res);
  unsigned neg_lit = NOT (lit);
  const unsigned plit = kissat_map_literal (solver, lit, true);
  const unsigned nlit = kissat_map_literal (solver, neg_lit, true);
  bool val = (plit == INVALID_LIT) || (nlit == INVALID_LIT);
  return val;
}
...
last_enqueued_unassigned_variable (kissat * solver)
{
  ...
  {
    res = (rand() % solver->vars) ;
    const value *values = solver->values;
    while (values[LIT (res)] || invalid_lit (solver, res))
      res = (rand() %solver->vars);
    solver->rand_selection++;
    //printf ("\n |vmtf| %d", solver->rand_selection );
  }
  else 
  {
...
kissat_decide (kissat * solver)
{
  ...
  if (solver->stable)
    solver->cdata.last_decision = idx;
