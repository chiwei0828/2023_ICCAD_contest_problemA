...
// mersenne twist
typedef struct hywalk_randgen hywalk_randgen;

struct hywalk_randgen
{
	unsigned mt[624];
	int 	 mti;
};
...
void 	init_HyWalk(HyWalk*);
void 	reinit_HyWalk(HyWalk*);
void 	confl_trans(HyWalk*);


bool    local_search(HyWalk*);
int     build_instance(HyWalk*,char *filename);
void    print_solution(HyWalk*);
int     verify_sol(HyWalk*);
void 	alloc_memory(HyWalk*);
void    free_memory(HyWalk*);
void    settings(HyWalk*, char*);
void    hywalk_merseene_init(HyWalk*, int);
void 	update_after_build(HyWalk*);
