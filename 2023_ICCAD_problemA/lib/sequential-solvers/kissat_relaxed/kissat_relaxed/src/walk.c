...
static unsigned
connect_binary_counters_relaxed (walker * walker)
{
  kissat *solver = walker->solver;
  value *values = solver->values;
  tagged *refs = walker->refs;
  ...
  //     if (!first_value || !second_value) // ( !0 || !0 ) = 1
	// continue;
      assert (counter_ref < walker->clauses);
      refs[counter_ref] = make_tagged (true, binary_ref);
      watches *first_watches = all_watches + first;
...
init_walker_relaxed (kissat * solver, walker * walker, litpairs * binaries)
{
  ...
  const unsigned counter_ref = connect_binary_counters_relaxed (walker);
...
//   const unsigned *lits = dereference_literals (solver, walker,
// 					       counter_ref, &size);
//   LOGLITS (size, lits, "checking large clause[%u]", counter->pos);
// #endif
    }
...
static void
local_search_step_relaxed (kissat * solver, walker * walker)
{
  assert (walker->current);
  INC (flipped);
#ifndef QUIET
  ...
  // else    printf("!!! skip fixed lit: %d\n", idx);
}

void update_ils_info(walker * walker,unsigned real_step){
  unsigned all_steps = 0;
  kissat *solver = walker->solver;
  infols *ils = &(solver->ils);
  for(unsigned unsat_idx=0;unsat_idx<SIZE_STACK(walker->unsat);++unsat_idx){
    const unsigned counter_ref = PEEK_STACK (walker->unsat, unsat_idx);
    unsigned size;
    ...
    for(const unsigned *p=lits;p!=end_of_lits;++p){
      const unsigned lit = *p;
      const unsigned var = IDX(lit);
      if(ils->last_updata_age[var]!=real_step){
        ils->conflicts_num[var]++;
        ils->last_updata_age[var]=real_step;
      }
    }
  }
  ADD(walk_steps, all_steps);
}
...
static void
local_search_round_relaxed (walker * walker, unsigned round)
{
  kissat *solver = walker->solver;
  kissat_very_verbose (solver,
		       "round %u starts with %u unsatisfied clauses",
  ...
  walker->limit = 500;
  // printf("!!! max_step: %ld\n", walker->limit);
  while (walker->minimum && walker->limit > walker->flipped) //walker->limit > statistics->walk_steps
  {
    if (TERMINATED (23))break;
    local_search_step_relaxed (solver, walker);
  }
...
walking_relaxed (kissat * solver)
{
  ...
  INC (walks);
  litpairs irredundant; // binary clauses
  litwatches redundant; // full clauses
  INIT_STACK (irredundant);
  INIT_STACK (redundant);

  kissat_enter_dense_mode (solver, &irredundant, &redundant);
  // printf("entered dense mode\n");
  walker walker;
  init_walker_relaxed (solver, &walker, &irredundant);
  const unsigned max_rounds = GET_OPTION (walkrelax);
  // printf("!!! max_rounds: %d\n", max_rounds);
  for (unsigned round = 1; walker.minimum && round <= max_rounds; round++)
    {
      if (TERMINATED (24))
	break;
      local_search_round_relaxed (&walker, round);
    }
  // save infos
  end_walked = solver->values;
  // clear
  solver->values = kissat_calloc (solver, LITS, 1);
  // printf("*** walker.current: %d\n", walker.current);
  // printf("*** walker.minimum: %d\n", walker.minimum);
...
static char
walking_phase_ils (kissat * solver)
{
  INC (walks);
  litpairs irredundant;
  litwatches redundant;
  ...
  while (walker.minimum && walker.limit > statistics->walk_steps)
  {
    ...
    if (TERMINATED (23))break;
    local_search_step (solver, &walker);
    //update ls information
    update_ils_info(&walker,real_step);
  }
  solver->ils.ls_steps = real_step;
  infols_get_confl_num(solver);
  solver->freeze_ils_gap = solver->options.aa_freeze_ils;
  char res = save_final_minimum (&walker);
  release_walker (&walker);
  kissat_resume_sparse_mode (solver, false, &irredundant, &redundant);
  RELEASE_STACK (irredundant);
...
kissat_walk (kissat * solver)
{
  ...
  char res;
  if(solver->stable && solver->freeze_ils_gap < 0)
    res = walking_phase_ils (solver);
  else
    res = walking_phase (solver);
...
bool
kissat_walk_relaxed (struct kissat * solver) {
  solver->level = 0;
  assert (!solver->inconsistent);
  assert (kissat_propagated (solver));
  STOP_SEARCH_AND_START_SIMPLIFIER (walking);
  ...
  STOP_SIMPLIFIER_AND_RESUME_SEARCH (walking);
  // printf("stoped simplifier and resume search\n");
  return res;
}