...
struct Internal {
  ...
  int pl[2] = {10,10},ts[2] = {0,0};
  int64_t nt[2] = {10000,10000};

  void Mt(Clause * c,bool f){
    if(f) 
      c->keep = true;
    else
      c->used = 2;
  ...
  string Gs(Clause * c, int * ss, bool f){
    int s = c->size,n = 0,l[s],li,i;
    for (i=0; i<s; i++){
      li = c->literals[i];
      if(!f || !vtab[vidx(li)].reason)
	l[n++] = li;
    }
    *ss = n;
    f?sort(l, l + n, [this] (int a, int b) {return var(a).level<var(b).level; } ):sort(l, l + n);
    
    string t = f?"":"K";
    for (i=0; i<n; i++) t = t + to_string(l[i]) + "|";
    return t;
  } 

  void DL(Clause * c, int f){    
    bool F = (f!=1) ? 1:0;
    
    if (c->glue  > 12 && c->keep == true)
      return;
    
    int l;
    string s = Gs(c,&l,F?0:1);

    if(!F && (l<2 || l>9))
      return;
    int n = dt.Cal(s);

    bool f1 = F?(n==2):(n  >=  pl[0]), f2 = F?(n>=3):(l>=3?(n >= 5  + pl[1]):(n >= 10  + pl[1]));
