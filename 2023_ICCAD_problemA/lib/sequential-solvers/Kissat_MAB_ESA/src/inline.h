...
kissat_update_variable_score(kissat* solver, heap* schedule, unsigned idx)
{
    ...
    if (solver->eliminate_by_activity)
    {
        new_score = kissat_get_heap_score(solver->heuristic == 0 ? &solver->scores : &solver->scores_chb, idx);
        LOG("new elimination score %g for variable %u.", new_score, idx);
    ...
    else
    {
        ...
        new_score = ((double)pos) * neg + pos + neg;
