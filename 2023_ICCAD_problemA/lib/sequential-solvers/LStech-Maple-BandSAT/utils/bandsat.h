...
// mersenne twist
typedef struct bandsat_randgen bandsat_randgen;

struct bandsat_randgen
{
	unsigned mt[624];
	int 	 mti;
};
...
typedef struct BandSAT BandSAT;
...
void 	init_BandSAT(BandSAT*);
void 	reinit_BandSAT(BandSAT*);
void 	confl_trans(BandSAT*);


bool    local_search(BandSAT*);
int     build_instance(BandSAT*,char *filename);
void    print_solution(BandSAT*);
int     verify_sol(BandSAT*);
void 	alloc_memory(BandSAT*);
void    free_memory(BandSAT*);
void    settings(BandSAT*, char*);
void    bandsat_merseene_init(BandSAT*, int);
void 	update_after_build(BandSAT*);
