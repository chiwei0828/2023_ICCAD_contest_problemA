...
void Internal::rephase () {
  ...
  if(!count)type=rephase_original();
    ...
    int p=rand()%1000;
    if((p-=400)<0)type=rephase_best();
    else if((p-=350)<0)type=rephase_walk();
    else if((p-=100)<0){for(auto idx:vars)if((val=phases.min[idx]))phases.saved[idx]=val;}
    else if((p-=50)<0)type=rephase_random();
    else if((p-=40)<0)type=rephase_original();
    else if((p-=10)<0)type=rephase_inverted();
    if(rand()%100<20)type=rephase_flipping();
  }
  /*if (single && !opts.walk) {
