...
OPTION( sweep, 0, 0, 1, "enable SAT sweeping") \
...
OPTION( vivifyeffort, 1, 0, 1e3, "effort in per mille") \
...
OPTION( walkahead, 0, 0, 1, "enable look-ahead in local search") \
OPTION( walkeffort, 1000, 0, 1e6, "effort in per mille") \
OPTION( walkfit, 1, 0, 1, "fit CB value to average clause length") \
OPTION( walkgood, 0, 0, 1, "pick good lit fristly in walking") \
OPTION( walkinitially, 0, 0, 1, "initial local search") \
OPTION( walkmethod, 0, 0, 2, "0: break value; 1: make value; 2: diff value")\
OPTION( walkreuse, 1, 0, 2, "reuse walking results (2=always)") \
OPTION( walksample, 0, 0, 1, "use sampling method instead of score random") \
OPTION( walkup, 0, 0, 1, "use unit propagation initial value") \
