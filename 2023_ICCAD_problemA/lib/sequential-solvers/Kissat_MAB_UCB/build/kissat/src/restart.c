...
void restart_mab(kissat * solver){   
	...
	if(stable_restarts < solver->mab_heuristics) {
		...
		for(unsigned i=0;i<solver->mab_heuristics;i++) {
		     if(solver->mab_strategy==0)
		     	ucb[i] = solver->mab_reward[i]/solver->mab_select[i] + sqrt(solver->mabc*log(stable_restarts+1)/solver->mab_select[i]);
		     else ucb[i] = solver->mab_reward[i]/solver->mab_select[i] + sqrt((solver->mabc/solver->mab_select[i])*log(fmax(1.0,(stable_restarts+1)/(2.0*solver->mab_select[i])))); 
