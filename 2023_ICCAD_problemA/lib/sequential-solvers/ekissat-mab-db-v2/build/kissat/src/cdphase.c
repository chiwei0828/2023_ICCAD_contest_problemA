...
void track_cdcbphase (kissat * solver)
{
    ...
    double u2v = (double) solver->unassigned / (double) solver->vars;
    if (solver->cdata.substantial_cd_phase 
        && solver->cdata.confs_since_restart) // apply action if in a substantial CD phase and we have seen at least a conflict after the current restart.
    {   
        if (u2v >= 0.1)
        {
